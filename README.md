...
이 리포지토리는 Walmart의 고객 리뷰를 スクレイピング, 처리 및 분석하기 위한 도구를 제공합니다. 이 시스템은 벡터 스토리지로 Pinecone을, 시맨틱 검색을 위해 Sentence Transformers를, 그리고 리뷰 추출 및 분석을 위해 Google의 Gemini AI를 활용합니다.
...
- **`walmart_review_extractor.py`**: Bright Data를 사용하여 Walmart에서 리뷰를 スクレイピング하고 Google Gemini AI를 사용하여 처리합니다.
...
4. 처리할 수 있도록 리뷰 データセット 파일(`extracted_walmart_reviews.json`)을 프로젝트 루트에 배치합니다.
